    ...
    def decorator(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                if not await room.users.filter(id=user.id).aexists(): # noqa501
                    raise DenyConnection(f"Usuário {user} não pertence à sala {room_id}") # noqa501
            except Room.DoesNotExist:
                raise DenyConnection(f"Sala {room_id} não existe.") # noqa501
